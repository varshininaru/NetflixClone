          ...
          {results.map((result) => {
            ...
            return (
              <div key={result.id} className='bg-gray-800 p-4 rounded'>
                {activeTab === "person" ? (
                  // For person, just show details, no Link
                  <div className='flex flex-col items-center'>
                    ...
                    {/* Optional: Add more person details if available */}
                    {result.known_for_department && (
                      <p className='text-sm text-gray-400'>Known for: {result.known_for_department}</p>
                    )}
                    {result.popularity && (
                      <p className='text-sm text-gray-400'>Popularity: {result.popularity.toFixed(1)}</p>
                    )}
                  </div>
                ) : (
                  // For movie/tv, keep the Link
